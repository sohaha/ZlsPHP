 ...
 * @since         v2.1.18
 * @updatetime    2018-4-21 20:06:28
 */
defined('IN_ZLS') || define("IN_ZLS", true);
...
class Z
{
    ...
    public static function &config($configName = null, $caching = true)
    {
        if (empty($configName)) {
    ...
    public static function arrayGet($arr, $key, $default = null, $explode = true)
    {
    ...
    public static function &db($group = '', $isNewInstance = false)
    {
        if (is_array($group)) {
            ...
            Z::throwIf(empty($config), 'Database', 'database configuration is empty , did you forget to use "->setDatabseConfig()" in index.php ?');
            if (empty($group)) {
    ...
    public static function getOpt($key = null)
    {
        if (!self::isCli()) {
            return null;
        }
        ...
        return empty($key) ? $result : (self::arrayKeyExists($key, $result) ? $result[$key] : null);
    }
    ...
    public static function host($prefix = true, $uri = false, $query = false)
    {
        ...
        if (!!$uri) {
            ...
            if (!$path) {
                $path = strstr(self::server('SCRIPT_NAME'), ZLS_PATH . '/' . ZLS_INDEX_NAME, true) . self::arrayGet($_SERVER, 'PATH_INFO', self::arrayGet($_SERVER, 'REDIRECT_PATH_INFO'));
            }
            $host .= $path;
        }
        if ($query === true) {
    ...
    public static function checkValue($value = '', $rule = [], $db = null)
    {
        ...
        return self::checkData(['value' => $value], ['value' => $rules], $redata, $_err, $_errKey, $db);
    }
    public static function checkData($data = [], $rules = [], &$returnData = null, &$errorMessage = '', &$errorKey = null, &$db = null)
    {
        static $checkRules;
        if (empty($checkRules)) {
            $defaultRules = (class_exists('\Zls\Util\CheckRules')) ? z::extension('Util_CheckRules')->getRules() : [];
            $userRules = self::config()->getDataCheckRules();
            $checkRules = (!empty($userRules) && is_array($userRules)) ? array_merge($defaultRules, $userRules) : $defaultRules;
        }
    ...
    public static function page($total, $page = 1, $pagesize = 10, $url = '{page}', $a_count = 6)
    {
        ...
        $result = ['pages' => [], 'total' => $total, 'count' => $pages, 'curpage' => $curpage, 'prefix' => $curpage == 1 ? '' : str_replace('{page}', $curpage - 1, $url), 'start' => str_replace('{page}', 1, $url), 'end' => str_replace('{page}', $pages, $url), 'subfix' => ($curpage == $pages || $pages == 0) ? '' : str_replace('{page}', $curpage + 1, $url),];
        for ($i = $start; $i <= $end; $i++) {
    ...
    public static function redirect($url, $msg = null, $time = 3, $view = null)
    {
        ...
        if (empty($msg) && empty($view)) {
            ...
            self::header("refresh:{$time};url={$url}");
            if (!empty($view)) {
    ...
    private static function getEncryptKey($key, $attachKey)
    {
        ...
        if (!$key && !$_key) {
            throw new \Zls_Exception_500('encrypt key can not empty or you can set it in index.php : ->setEncryptKey()');
        }
    ...
    public static function isWhiteIp($clientIp)
    {
        ...
        foreach ($config->getMaintainIpWhitelist() as $ip) {
            ...
            if (Z::ipInfo($clientIp . '/' . $netmask, 'netaddress') == Z::ipInfo($info[0] . '/' . $netmask, 'netaddress')) {
                $isWhite = true;
                break;
            }
        }
...
class Zls
{
    ...
    public static function run()
    {
        ...
        $config->setApplicationDir(ZLS_APP_PATH);
    ...
    private static function runCli()
    {
        self::initDebug();
        $executes = ['_execute'];
        $task = str_replace('/', '_', Z::getOpt('task'));
        if (empty($task)) {
            $task = Z::getOpt('1') ?: (Z::getOpt('make') ? 'make' : null);
        }
        if (empty($task)) {
            echo 'Perform a task, please use -task <taskName> (php zls -task xxx)' . PHP_EOL;
            echo 'Perform a artisan, please command artisan <artisanName> (php zls artisan xxx)' . PHP_EOL;
            Z::finish();
        }
        $hmvcModuleName = Z::getOpt('hmvc');
        if (!empty($hmvcModuleName)) {
            self::checkHmvc($hmvcModuleName);
        }
        $taskName = \Zls::getConfig()->getTaskDirName() . '_' . $task;
        $taskObject = null;
        $args = Z::factory('Zls_CliArgs');
        try {
            ...
            if (!class_exists($taskName)) {
                if ($task === 'make') {
                    $taskName = 'Task_Artisan_Main';
                    $taskObject = z::factory($taskName, true);
                } elseif ($task === 'artisan') {
                    $artisanTask = ucfirst(Z::getOpt('2'));
                    $artisan = '';
                    $executes = $args->get('h') ? ['help'] : [];
                    if (empty($artisanTask)) {
                        if (!$executes) {
                            Z::finish('Warning: Unknown artisan command' . PHP_EOL);
                        } else {
                            $taskName = 'Task_Artisan_Main';
                        }
                    } else {
                        $artisanTask = explode(':', $artisanTask);
                        $artisan = array_shift($artisanTask);
                        if (!!$artisanTask) {
                            $executes = $executes + $artisanTask;
                        }
                        $taskName = $artisan === 'Create' ? 'Task_Artisan_Main' : 'Artisan_' . $artisan;
                    }
                    try {
                        $taskObject = z::factory($taskName, true);
                    } catch (\Zls_Exception_500 $e) {
                        Z::throwIf($e->getErrorType() !== 'ERROR', 500, $e->getMessage());
                        z::finish("Artisan [ {$artisan} ] not found" . PHP_EOL);
                    }
                } else {
                    $taskObject = z::factory($taskName, true);
                }
        ...
        foreach ($executes as $execute) {
            if ($execute === 'help' && !method_exists($taskObject, $execute)) {
                continue;
            }
    ...
    public static function runWeb($result = false)
    {
        self::initDebug();
        $config = Z::config();
        $_apiDoc = (isset($_GET['_api']) && !!$config->getApiDocToken() && (Z::get('_token', '', true) === $config->getApiDocToken()) && class_exists('Zls\Util\ApiDoc'));
        $config = self::getConfig();
        $class = '';
        $method = '';
        ...
        if (!$contents) {
            ...
            if (!!$_apiDoc) {
                $docComment = Z::extension('Util_ApiDoc');
                if ($_GET['_api'] == 'self') {
                    $docComment::html('self', $docComment::apiMethods($docComment::getClassName($class), $method, true, $config->getRoute()->gethmvcModuleName()));
                } else {
                    if ($_GET['_api'] == 'all') {
                        $docComment::html('parent', $docComment::all());
                    } else {
                        $docComment::html('parent', $docComment::docComment($docComment::getClassName($_route->getController()), $config->getRoute()->gethmvcModuleName()));
                    }
                }
                return false;
            }
            ...
            if (!empty($cacheMethodConfig) && Z::arrayKeyExists($methodKey, $cacheMethodConfig) && $cacheMethodConfig[$methodKey]['cache'] && ($cacheMethoKey = $cacheMethodConfig[$methodKey]['key']())) {
...
class Zls_Database_ActiveRecord extends Zls_Database
{
    ...
    private $arSelect, $arJoin, $arWhere, $arGroupby, $arHaving, $arLimit, $arOrderby, $arSet, $primaryKey, $arUpdateBatch, $arInsert, $arInsertBatch, $_asTable, $_asColumn, $_values, $_sqlType, $_currentSql;
...
{
    public final function toArray($fields = [])
    {
...
{
    private $db, $rs, $_cacheTime = null, $_cacheKey;
    ...
    public function getPage($page = 1, $pagesize = 10, $url = '{page}', $fields = '*', array $where = null, array $orderBy = [], $pageBarACount = 6)
    {
        $data = [];
        if (is_array($where)) {
            $this->getDb()->where($where);
    ...
    public function search($page = 1, $pagesize = 10, $url = '{page}', $fields = '*', $cond = '', array $values = [], $pageBarACount = 10)
    {
        $data = [];
...
{
    private $driverType, $database, $tablePrefix, $pconnect, $debug, $timeout, $trace, $charset, $collate, $tablePrefixSqlIdentifier, $slowQueryTime, $slowQueryHandle, $slowQueryDebug, $minIndexType, $indexDebug, $indexHandle, $masters, $slaves, $resetSql, $attribute, $connectionMasters, $connectionSlaves, $versionThan56 = false, $_errorMsg, $_lastSql, $_lastPdoInstance, $_isInTransaction = false, $_config, $_lastInsertId = 0, $_traceRes = [], $_cacheTime = null, $_cacheKey, $_masterPdo = null, $_locked = false;
    public function __construct(array $config = [])
    {
    ...
    public function getDefaultConfig()
    {
        return [
            ...
            'indexHandle'              => null,
            'attribute'                => [],
            'masters'                  => ['master01' => ['hostname' => '127.0.0.1', 'port' => 3306, 'username' => 'root', 'password' => '',],],
            'slaves'                   => [],
        ];
    }
    ...
    {
        $info = ['master' => ['getMasters', 'connectionMasters',], 'slave' => ['getSlaves', 'connectionSlaves',],];
        try {
    ...
    protected function _displayError($message, $code = 0)
    {
        ...
        if ($this->getDebug() || $this->_isInTransaction) {
            if ($message instanceof Exception) {
                throw new \Zls_Exception_Database($group . $this->_errorMsg, 500, 'Zls_Exception_Database', $message->getFile(), $message->getLine());
            } else {
    ...
    public function execute($sql = '', array $values = [])
    {
        ...
        if ($this->slowQueryDebug || $this->indexDebug) {
            $trace = Z::tap(debug_backtrace(), function (&$trace) {
                ...
                $trace = ['file' => $_trace['file'], 'line' => $_trace['line'], 'class' => $_trace['class'], 'function' => $_trace['function'],];
            });
        }
        if (!$this->_init()) {
            return false;
        ...
        try {
            ...
            if ($this->indexDebug && $this->indexHandle instanceof Zls_Database_Index_Handle) {
                $badIndex = false;
                if ($this->_isMysql()) {
                    $order = ['system' => 1, 'const' => 2, 'eq_ref' => 3, 'ref' => 4, 'fulltext' => 5, 'ref_or_null' => 6, 'index_merge' => 7, 'unique_subquery' => 8, 'index_subquery' => 9, 'range' => 10, 'index' => 11, 'all' => 12,];
                    foreach ($explainRows as $row) {
                        if (Z::arrayKeyExists(strtolower($row['type']), $order) && Z::arrayKeyExists(strtolower($this->getMinIndexType()), $order)) {
...
abstract class Zls_Task
{
    ...
    abstract function execute(\Zls_CliArgs $args);
...
class Zls_Router_PathInfo extends Zls_Router
{
    ...
    public function url($action = '', $getData = [], $opt = ['subfix' => true, 'ishmvc' => false])
    {
        ...
        if ($isHmvc === true) {
            $hmvcModules = $config->getHmvcModules();
            $hmvcDirName = !!Z::arrayGet($hmvcModules, $config->getRoute()->getHmvcModuleName(), null) ? $config->getRoute()->getHmvcModuleName() : '';
...
abstract class Zls_Artisan extends Zls_Task
{
    public function _execute(\Zls_CliArgs $args)
    {
        $this->execute($args);
...
abstract class Zls_Exception extends \Exception
{
    ...
    {
        $run = z::debug(false);
        return '<html><meta name="viewport" content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0"><body style="line-height:30px;padding:0;margin:0;background:#0C8611;color:whitesmoke;font-family:\'Courier New\',monospace;font-size:18px;">' . '<div style="padding:10px;background:#104411;color:#4CAF50;font-size:25px;font-weight:bold;">' . $this->exceptionName . ' - [ ' . $this->getErrorType() . ' ] </div>' . '<div style="padding:10px;color:yellow;">' . '<strong>Environment: </strong>
' . $this->getEnvironment() . '<br/>' . '<strong>Line: </strong>' . $this->getErrorLine() . ' [ ' . $this->getErrorFile(true) . ' ]<br/>' . '<strong>Message: </strong>' . htmlspecialchars($this->getErrorMessage()) . '</br>' . '<strong>Time: </strong>' . date('Y/m/d H:i:s T') . '</br>' . '<strong>WasteTime: </strong>' . $run['runtime'] . '</br>' . '<strong>Memory: </strong>' . $run['memory'] . '</div>' . '</body></html>';
    }
    ...
    {
        $value = $errorType;
        $levelNames = [E_ERROR => 'ERROR', E_WARNING => 'WARNING', E_PARSE => 'PARSE', E_NOTICE => 'NOTICE', E_CORE_ERROR => 'CORE_ERROR', E_CORE_WARNING => 'CORE_WARNING', E_COMPILE_ERROR => 'COMPILE_ERROR', E_COMPILE_WARNING => 'COMPILE_WARNING', E_USER_ERROR => 'USER_ERROR', E_USER_WARNING => 'USER_WARNING', E_USER_NOTICE => 'USER_NOTICE',];
        if (defined('E_STRICT')) {
...
abstract class Zls_Session
{
    ...
    public abstract function init($sessionID);
    public abstract function swooleInit($sessionID);
    public abstract function swooleGet($key);
    public abstract function swooleUnset($key);
    public abstract function swooleSet($key, $value);
}
class Zls_Exception_404 extends Zls_Exception
{
    protected $exceptionName = 'Zls_Exception_404', $httpStatusLine = 'HTTP/1.0 404 Not Found';
...
{
    protected $exceptionName = 'Zls_Exception_500', $httpStatusLine = 'HTTP/1.0 500 Internal Server Error';
...
{
    protected $exceptionName = 'Zls_Exception_Database', $httpStatusLine = 'HTTP/1.0 500 Internal Server Error';
    public function __construct($errorMessage = '', $errorCode = 500, $errorType = 'Exception', $errorFile = '', $errorLine = '0')
    {
        parent::__construct($errorMessage, $errorCode, $errorType, $errorFile, $errorLine);
    }
...
class Zls_Request_Default implements Zls_Request
{
    ...
    public function __construct()
    {
        if (!$this->pathInfo = Z::arrayGet($_SERVER, 'PATH_INFO', Z::arrayGet($_SERVER, 'REDIRECT_PATH_INFO'))) {
            $this->pathInfo = parse_url(Z::arrayGet($_SERVER, 'REQUEST_URI', ''), PHP_URL_PATH);
        }
...
class Zls_View
{
        ...
        if ($hmvcDirName) {
            ...
            foreach ($trace as $t) {
                ...
                if (!empty($filepath)) {
                    ...
                    if (($filepath && in_array($function, $checkList) && strpos($filepath, $appPath) === 0 && strpos($filepath, $hmvcPath) === 0) || $function == 'handle') {
                        $calledIsInHmvc = true;
                        break;
    ...
    public function loadHmvc($hmvcModuleName, $viewName, $data = [], $return = false)
    {
        $config = Z::config();
        $hmvcDirName = Z::arrayGet($config->getHmvcModules(), $hmvcModuleName, $hmvcModuleName);
        $path = $config->getPrimaryApplicationDir() . $config->getHmvcDirName() . '/' . $hmvcDirName . '/' . $config->getViewsDirName() . '/' . $viewName . '.php';
        return $this->loadRaw($path, $data, $return);
    }
}
class Zls_CliArgs
            ...
            if (!!$router) {
                foreach ($router as $_router => $value) {
                    if (!isset($value['view']) && !isset($value['viewFile'])) {
                        ...
                        $routerData = $value;
                        break;
                    } elseif (!!$defaultMethod && preg_match('/^' . str_replace('/', '\/', $_router) . '$/', $document . '/' . $defaultMethod, $routerMatch)) {
...
class Zls_Config
{
    ...
    private $applicationDir = '', $primaryApplicationDir = '', $indexDir = '', $classesDirName = 'classes', $hmvcDirName = 'hmvc', $libraryDirName = 'library', $functionsDirName = 'functions', $storageDirPath = '', $viewsDirName = 'views', $configDirName = 'config', $controllerDirName = 'Controller', $businessDirName = 'Business', $daoDirName = 'Dao', $beanDirName = 'Bean', $modelDirName = 'Model', $taskDirName = 'Task', $defaultController = 'Index', $defaultMethod = 'index', $methodPrefix = 'z_', $methodUriSubfix = '.go', $methodParametersDelimiter = '-', $logsSubDirNameFormat = 'Y-m-d/H', $exceptionLevel = '', $exceptionControl = true, $cookiePrefix = '', $backendServerIpWhitelist = [], $isRewrite = true, $request, $showError = false, $traceStatus = false, $routersContainer = [], $packageMasterContainer = [], $packageContainer = [], $loggerWriters = [], $logsMaxDay = 10, $uriRewriter, $exceptionHandle, $route, $environment = 'development', $hmvcModules = [], $isMaintainMode = false, $maintainIpWhitelist = [], $maintainModeHandle, $databseConfig, $cacheHandles = [], $cacheConfig, $sessionConfig, $sessionHandle, $hvmcDomain, $methodCacheConfig, $dataCheckRules, $outputJsonRender, $exceptionJsonRender, $zMethods = [], $encryptKey, $apiDocToken = '', $exceptionMemoryReserveSize = 256000, $separationRouter = false, $traceStatusCallBack = null, $clientIpConditions = [
        'source' => [
    ...
    ], $hmvcDomains = ['enable' => false, 'domains' => [],];
    ...
    public function getCurrentDomainHmvcModuleNname()
    {
        ...
        foreach ($this->hmvcDomains['domains'] as $prefix => $hvmc) {
            ...
            if ((z::arrayGet($hvmc, 'isRegEx') === true) && (preg_match('/^' . $hvmcDomain . '$/', $_domain))) {
    ...
    public function getEnvironment()
    {
        if (empty($this->environment)) {
            $this->environment = ($env = (($cliEnv = Z::getOpt('env')) ? $cliEnv : Z::arrayGet($_SERVER, 'ENVIRONMENT'))) ? $env : 'production';
        }
    ...
    public function setEncryptKey($encryptKey)
    {
        ...
        if (is_array($encryptKey)) {
            ...
            $this->encryptKey = ['default' => $encryptKey,];
        }
        return $this;
    }
    ...
    public function getOutputJsonRender()
    {
        ...
        if (empty($this->outputJsonRender)) {
            $this->outputJsonRender = function () {
                ...
                $json = json_encode(['code' => $code, 'msg' => $message, 'data' => $data], JSON_UNESCAPED_UNICODE + JSON_UNESCAPED_SLASHES);
                if ($die) {
                    Z::finish($json);
                }
                return $json;
    ...
    public function getCacheHandle($key = '')
    {
        if (empty($this->cacheConfig)) {
            $this->cacheConfig = Z::config('cache') ?: ['default_type' => 'file', 'drivers' => ['file' => ['class' => 'Zls_Cache_File', 'config' => Z::config()->getStorageDirPath() . 'cache/',],],];
        }
        if (is_array($key)) {
    ...
    public function getSessionConfig()
    {
        if (empty($this->sessionConfig)) {
            $this->sessionConfig = ['autostart' => false, 'cookie_path' => '/', 'cookie_domain' => Z::arrayGet(explode(':', Z::server('HTTP_HOST')), 0, Z::server('HTTP_HOST')), 'session_name' => 'ZLS', 'lifetime' => 86400, 'session_save_path' => null,];
        }
    ...
    {
        if (is_string($maintainIpWhitelist)) {
            $this->maintainIpWhitelist = Z::config($maintainIpWhitelist);
        } else {
...
class Zls_Logger_Dispatcher
{
    ...
    final public function dispatch(\Zls_Exception $exception, $result = false)
    {
        ...
        if ($config->getShowError()) {
            ...
            $path = [$config->getApplicationDir() . $config->getViewsDirName() . '/error/' . $exception->getErrorCode() . '.php', $config->getPrimaryApplicationDir() . $config->getViewsDirName() . $exception->getErrorCode() . '.php',];
...
class Zls_Cache_File implements Zls_Cache
{
    ...
    private function unpack($cacheData)
    {
        ...
        if (is_array($cacheData) && Z::arrayKeyExists('userData', $cacheData) && Z::arrayKeyExists('expireTime', $cacheData)) {
    ...
    private function pack($userData, $cacheTime)
    {
        ...
        return @serialize(['userData' => $userData, 'expireTime' => ($cacheTime == 0 ? 0 : time() + $cacheTime),]);
    }
    public function &instance($key = null, $isRead = true)
    {
        return $this;
...
class Zls_Trace
{
    ...
    public function log(array $data, $type = 'trace')
    {
        ...
        $this->output(vsprintf("traceType : %s\ntime : %s\nruntime : %s\nmemory : %s\npath : %s\nline : %u\nargs : \n%s\n\n", ['log', date('Y-m-d H:i:s'), $debug['runtime'] . 's', $debug['memory'], \Z::safePath($data['file']), $data['line'], $arg,]), $type);
    }
}
